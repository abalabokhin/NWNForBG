...
~  Global("NWNChapter","GLOBAL",2)Global("FindCult","GLOBAL",0)Global("FindCultTalk","LOCALS",0)~ THEN BEGIN 45 // from:
  SAY ~Итак, город спасен от воющей смерти. Но эта болезнь не случайно возникла в Невервинтере, это происки какого-то таинственного культа, в котором состоял Дестер. Леди Арибет уже отправилась на поиски этого культа, сейчас ее штаб располагается в Порт-Лласте, это небольшой городок к северу отсюда. Я хочу, чтобы вы помогли найти этот культ, зло должно быть наказано. Отправляйтесь в Порт-Лласт и найдите злодеев.~
  IF ~~ THEN REPLY ~Зачем надо было казнить Фентика?.~ GOTO 46
  IF ~~ THEN REPLY ~Как я найду этот город?~ GOTO 47
  IF ~~ THEN REPLY ~Я немедленно отправляюсь в Порт-Лласт.~ GOTO 48
  IF ~~ THEN REPLY ~Хватит с меня ваших проблем, я не хочу вам помогать.~ GOTO 49
END

IF ~~ THEN BEGIN 46 // 45.1
  SAY ~Он помогал предателю, и народ потребовал его смерти. Если бы я помиловал Фентика, жители Невервинтера могли устроить бунт, они достаточно настрадались от этой чумы.~
  IF ~~ THEN REPLY ~Как мне найти Порт-Лласт?~ GOTO 47
  IF ~~ THEN REPLY ~Я немедленно отправляюсь в Порт-Лласт.~ GOTO 48
  IF ~~ THEN REPLY ~Хватит с меня ваших проблем, я не хочу вам помогать.~ GOTO 49
END

IF ~~ THEN BEGIN 47 // 45.2
  SAY ~Вы найдете его на карте, это к северу от Невервинтера.~
  IF ~~ THEN REPLY ~Зачем надо было казнить Фентика?.~ GOTO 46
  IF ~~ THEN REPLY ~Я немедленно отправляюсь в Порт-Лласт.~ GOTO 48
  IF ~~ THEN REPLY ~Хватит с меня ваших проблем, я не хочу вам помогать.~ GOTO 49
END
...
IF ~~ THEN BEGIN 48 // 45.3
  SAY ~Я рад, что вы согласились помочь, удачи вам!~
  IF ~~ THEN DO ~SetGlobal("FindCult","GLOBAL",1)RevealAreaOnMap("NW2000")~ UNSOLVED_JOURNAL ~Поиски культа

Благодаря нашим усилиям город Невервинтер был спасен, и оставшимся в живых жителям больше не угрожает Воющая смерть. Однако Дестер, человек, распространявший чуму, действовал не в одиночку. Значительную помощь и поддержку ему оказывал могущественный загадочный культ, о котором было известно только то, что он располагается где-то к северу от Невервинтера. Леди Арибет перенесла свой штаб расследования в поселение в Порт-Лласте, этот городок - самый северный член Союза Лордов. Она ждет нас в этом городе.~ EXIT
END

IF ~~ THEN BEGIN 49 // 45.4
  SAY ~Ну что ж, очень жаль. Я не могу вам приказывать.~
...
~  Global("NWNChapter","GLOBAL",2)Global("FindCult","GLOBAL",0)Global("FindCultTalk","LOCALS",1)~ THEN BEGIN 50 // from:
  SAY ~Зачем вы вернулись, вы передумали? Мне очень нужно найти этот культ. От леди Арибет пока никаких известий нет, она все еще нуждается в вас. Если хотите помочь отправляйтесь в Порт-Лласт и поговорите с леди Арибет.~
  IF ~~ THEN REPLY ~Зачем надо было казнить Фентика?.~ GOTO 46
  IF ~~ THEN REPLY ~Как я найду этот город?~ GOTO 47
  IF ~~ THEN REPLY ~Я немедленно отправляюсь в Порт-Лласт.~ GOTO 48
  IF ~~ THEN REPLY ~Хватит с меня ваших проблем, я не хочу вам помогать.~ GOTO 49
...
~  Global("NWNChapter","GLOBAL",2)GlobalGT("FindCult","GLOBAL",0)~ THEN BEGIN 51 // from:
  SAY ~Насколько мне известно, культ еще не найден. Давайте оставим все эти бесполезные разговоры, займитесь поиском культа.~
  IF ~~ THEN EXIT
END
