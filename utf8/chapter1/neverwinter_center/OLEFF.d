...
IF ~~ THEN BEGIN 5 // from: 1.1
  ...
  IF ~  PartyHasItem("Nwmisc18")~ THEN REPLY ~Рассказать, как забрали благословленное перо Джейкоба у злобного Джиллса.~ GOTO 10
...
IF ~~ THEN BEGIN 33 // from: 31.2
  SAY ~Я очень рад, что вы предложили свою помощь. Вы не могли появиться в более подходящее время. Вот, возьмите письмо с моей печатью. Это значит, что вы действуете по моему приказу. Отнесите это Брайли на полуостров, и он поможет вам. Он охраняет уже найденную могилу. Это в подвале одного из домов на востоке района полуострова. Возвращайтесь, если найдете что-нибудь, а уж я в долгу не останусь.~
  IF ~~ THEN DO ~SetGlobal("OleffQuest","GLOBAL",1)GiveItemCreate("OleffL1",LastTalkedToBy,1,0,0)~ UNSOLVED_JOURNAL ~Храм Тира: Могила Невера
...
IF ~~ THEN BEGIN 38 // from: 36.2
  ...
  IF ~  PartyHasItem("Nwmisc18")~ THEN REPLY ~Рассказать, как отобрали благословленное перо Джейкоба у злобного Джиллса.~ GOTO 43
  IF ~~ THEN REPLY ~Извини, это была шутка.~ GOTO 11
END
...
Все три артефакта от Лорда Невера были возвращены Судье Олеффу в Зал Справедливости. Они должны помочь поднять дух пораженного чумой города, а также могут принести некоторую практическую пользу.~ EXIT
END
...
Артефакты из гробниц Хэлуэта Невера и его спутников были найдены. К сожалению, некоторые из них попали в руки соперничающей церкви. Олеффа, естественно, встревожил этот факт, хотя ни он, ни кто-либо другой теперь ничего не сможет с этим поделать. Неизвестно зачем, но зло приложило к этому руку.~ EXIT
END
...
IF ~~ THEN BEGIN 59 // from: 58.1
  ...
  IF ~  PartyHasItem("OphalLet")~ THEN REPLY ~У меня к вам письмо от Офалы.~ GOTO 6
  IF ~  PartyHasItem("Nwmisc18")~ THEN REPLY ~Рассказать, как забрали благословленное перо Джейкоба у злобного Джиллса.~ GOTO 60
  IF ~~ THEN REPLY ~Мне нужны услуги храма.~ GOTO 9
  IF ~~ THEN REPLY ~До свидания.~ GOTO 4
END
...
~  Global("OleffQuest","GLOBAL",2)~ THEN BEGIN 61 // from:
  ...
  IF ~  PartyHasItem("OphalLet")~ THEN REPLY ~У меня к вам письмо от Офалы.~ GOTO 6
  IF ~  PartyHasItem("Nwmisc18")~ THEN REPLY ~Рассказать, как забрали благословленное перо Джейкоба у злобного Джиллса.~ GOTO 62
  IF ~~ THEN REPLY ~Мне нужны услуги храма.~ GOTO 9
  IF ~~ THEN REPLY ~До свидания.~ GOTO 4
END
