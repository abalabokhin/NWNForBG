...
IF ~~ THEN BEGIN 1 // from: 3.0 0.2 0.1 0.0
  ...
  IF ~~ THEN REPLY ~Я сдаюсь. Не хочу больше этих загадок, верни меня в таверну.~ GOTO 5
...
IF ~~ THEN BEGIN 5 // from: 1.3
  SAY ~Вы уверены? Вы не сможете вернуться, вернуться обратно.~
  IF ~~ THEN REPLY ~Скажи нам еще раз, кого ты пропустишь?~ GOTO 1
  IF ~~ THEN REPLY ~Все решено, я возвращаюсь в таверну.~ GOTO 6
  IF ~~ THEN REPLY ~Я просто заглянул. До встречи.~ ~Я просто заглянула. До встречи.~ EXIT
END
