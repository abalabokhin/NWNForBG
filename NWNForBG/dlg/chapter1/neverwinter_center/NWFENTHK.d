// Центр Невервинтера  Зал справедливости   Фентик эльф   Фентик очень озабочен судьбой больных, он готов на все, лишь бы облегчить их страдания. Он выглядит уставшим, но полным надежды. + NWDESTHR
...
~  NumTimesTalkedTo(0)Global("NWNChapter","GLOBAL",0)IsGabber(Player1)~ THEN BEGIN 0 // from:
  SAY @0
  IF ~~ THEN EXTERN ~NWDESTHR~ 2 // Это ошибка, Фентик. Этот <CLASS> нам незнаком! Как вы можете проявлять такое слепое доверие?
END

IF ~~ THEN BEGIN 1 // from: NWDESTHR 2
  SAY @1
  IF ~~ THEN REPLY @2 EXTERN ~NWDESTHR~ 3 // А я говорю, что нужно довериться моим хельмитам, <GIRLBOY> ничего не сможет!
  IF ~~ THEN REPLY @3 EXTERN ~NWDESTHR~ 4 // Какое вопиющее неуважение к страданиям других! И такие наемники должны на нас работать, Фентик?
  IF ~~ THEN REPLY @4 EXTERN ~NWDESTHR~ 4 // Какое вопиющее неуважение к страданиям других! И такие наемники должны на нас работать, Фентик?
END
...
IF ~~ THEN BEGIN 6 // from: 4.1
  SAY @13
  IF ~~ THEN EXTERN ~NWDESTHR~ 5 // Нападение на академию лишь ухудшило положение дел в последние несколько недель, Фентик - дайте только людям узнать, как Арибет потеряла лекарство из-за недостатка веры!
END
...
IF ~~ THEN BEGIN 9 // from: 7.2
  ...
  IF ~~ THEN REPLY @24 EXTERN ~NWDESTHR~ 10 // <HESHE> говорит так, словно меня здесь нет! Неважно, все равно я слишком занят - нет времени болтать с вами.
END
...
IF ~~ THEN BEGIN 11 // from: 9.2
  SAY @27
  IF ~~ THEN REPLY @28 EXTERN ~NWDESTHR~ 6 // Нас хранит наша вера! Лишь маловеры боятся Воющей Смерти, я и мои хельмиты все время это говорим.
  IF ~~ THEN REPLY @26 GOTO 16
END

IF ~~ THEN BEGIN 12 // from: 9.3
  SAY @29
  IF ~~ THEN EXTERN ~NWDESTHR~ 8 // Пусть <CHARNAME> узнает правду, Фентик! У Арибет было лекарство, но она потеряла его, потому что доверилась этой Академии, а не моим собратьям-хельмитам!
END
...
IF ~~ THEN BEGIN 14 // from: 9.5
  SAY @31
  IF ~~ THEN EXTERN ~NWDESTHR~ 9 // Вы позволяете вашим чувствам взять над вами верх, Фентик. Ваша любовница отнюдь не совершенна.
END
...
IF ~~ THEN BEGIN 17 // from: NWDESTHR 6
  SAY @34
  IF ~~ THEN EXTERN ~NWDESTHR~ 7 // Паника усилится, когда люди узнают о нападении на Академию и о том, как Арибет потеряла лекарство.
END
...
IF ~~ THEN BEGIN 24 // from: 22.2
  SAY @44
  IF ~~ THEN EXTERN ~NWDESTHR~ 11 // Что может быть хуже Воющей Смерти, Фентик? Боюсь, <CHARNAME> обманывает вас этим 'доказательством' какого-то большого заговора!
END
...
IF ~~ THEN BEGIN 26 // from: NWDESTHR 12 NWDESTHR 13
  SAY @45
  IF ~~ THEN EXTERN ~NWDESTHR~ 14 // Чума - это испытание, ниспосланное богами, чтобы проверить нашу веру, а не часть заговора против Невервинтера. Проверять все эти теории заговора - пустая трата времени!
END
...
IF ~~ THEN BEGIN 31 // from: 22.3
  SAY @52
  IF ~~ THEN EXTERN ~NWDESTHR~ 16 // Прежде чем начать терять время на расследование некоего таинственного культа, Фентик, вам понадобится больше доказательств. Все мы знаем, что эта бумажка поддельная.
END
...
IF ~~ THEN BEGIN 37 // from: 22.4
  SAY @57
  IF ~~ THEN EXTERN ~NWDESTHR~ 19 // *Все* знали, что в Академии находятся уотердипские существа, Фентик - да даже сами студенты слышали эти сплетни! Нападение произошло из-за сплетен тех, кому Арибет доверилась вместо моих верных хельмитов. Искать этого призрачного шпиона - только зря время терять!
END
...
IF ~~ THEN BEGIN 43 // from: NWDESTHR 21
  SAY @62
  IF ~~ THEN EXTERN ~NWDESTHR~ 22 // Возможно... Но я предпочитаю верить Хельму и своим собратьям, этот <CLASS>, призванный Арибет не внушает доверия.
END
...
IF ~~ THEN BEGIN 46 // from: NWDESTHR 23
  SAY @65
  IF ~~ THEN EXTERN ~NWDESTHR~ 24 // Надежды? Мои хельмиты распространяют надежду и благословления по городу, Фентик. Мы также ищем пропавших существ. Лишь славы мы не ищем, как вот этот <CLASS>.
END
...
IF ~~ THEN BEGIN 49 // from: NWDESTHR 25
  SAY @67
  IF ~~ THEN EXTERN ~NWDESTHR~ 26 // Я и мои хельмиты предпочитаем оставаться преданными нашей вере, Фентик - не то, что некоторые. Мы ищем ответа у божественного Хельма, а не у смертных, как этот <CLASS>.
END
