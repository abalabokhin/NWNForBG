// Центр Невервинтера   Стражник у ворот в доки   Лицо этого стражника выражает усталость и напряжение после долгих часов борьбы с последствиями чумы.

BEGIN ~NWGUARD4~

IF WEIGHT #0 /* Triggers after states #: 9 even though they appear after this state */
~  Global("NWNDocksGate","MYAREA",0)Global("NWNDocksDone","GLOBAL",0)~ THEN BEGIN 0 // from:
  SAY @0
  IF ~  Global("NWNChapter","GLOBAL",1)~ THEN REPLY @1 DO ~SetGlobal("NWNDocksGate","MYAREA",1)~ GOTO 1
...
IF ~~ THEN BEGIN 16 // from: 1.1
  SAY @33
  IF ~  GlobalGT("NWNDocksDone","GLOBAL",0)~ THEN REPLY @34 GOTO 17
  IF ~  Global("NWNDocksDone","GLOBAL",0)~ THEN REPLY @34 GOTO 18
  IF ~  GlobalGT("NWNDocksDone","GLOBAL",0)~ THEN REPLY @35 GOTO 19
  IF ~  Global("NWNDocksDone","GLOBAL",0)~ THEN REPLY @35 GOTO 20
  IF ~  Global("NWNDocksDone","GLOBAL",0)~ THEN REPLY @36 GOTO 21
...
~  GlobalGT("NWNDocksGate","MYAREA",0)Global("NWNDocksDone","GLOBAL",0)~ THEN BEGIN 23 // from:
  SAY @46
  IF ~~ THEN REPLY @6 GOTO 16
  IF ~~ THEN REPLY @7 GOTO 5
  IF ~~ THEN REPLY @8 GOTO 6
...
~  GlobalGT("NWNDocksGate","MYAREA",0)GlobalGT("NWNDocksDone","GLOBAL",0)~ THEN BEGIN 24 // from:
  SAY @47
  IF ~~ THEN REPLY @6 GOTO 16
  IF ~~ THEN REPLY @7 GOTO 5
  IF ~~ THEN REPLY @8 GOTO 6
