...
IF ~~ THEN BEGIN 30 // from: 27.1 28.1
  SAY @59
  IF ~~ THEN DO ~EraseJournalEntry(@46)EraseJournalEntry(%Чернозерье: спасение Самуэля
...
IF ~~ THEN BEGIN 36 // from: 34.1
  SAY @69
  IF ~~ THEN DO ~SetGlobal("ThurinQuest","GLOBAL",4)GiveGoldForce(975)AddexperienceParty(90000)EraseJournalEntry(@46)EraseJournalEntry(%Чернозерье: спасение Самуэля

Стражник Самуэль, который ранее числился пропавшим без вести, был освобожден из плена в особняке Мелданена в Чернозерье. Он вернется в трактир "Голая Доска", чтобы отчитаться перед своим начальником Турином. Те, кто освободил его, должны поговорить с Турином по поводу вознаграждения.%)~ SOLVED_JOURNAL @70 EXIT
END

IF ~~ THEN BEGIN 37 // from: 34.2
  SAY @71
  IF ~~ THEN DO ~SetGlobal("ThurinQuest","GLOBAL",4)GiveGoldForce(800)AddexperienceParty(90000)EraseJournalEntry(@46)EraseJournalEntry(%Чернозерье: спасение Самуэля
...
IF ~~ THEN BEGIN 38 // from: 34.3
  SAY @72
  IF ~~ THEN DO ~SetGlobal("ThurinQuest","GLOBAL",4)GiveGoldForce(800)AddexperienceParty(50000)EraseJournalEntry(@46)EraseJournalEntry(%Чернозерье: спасение Самуэля
...
IF ~~ THEN BEGIN 40 // from: 34.5
  SAY @74
  IF ~~ THEN DO ~SetGlobal("ThurinQuest","GLOBAL",4)ReputationInc(1)AddexperienceParty(100000)EraseJournalEntry(@46)EraseJournalEntry(%Чернозерье: спасение Самуэля
