...
~  Global("ThurinQuest","GLOBAL",1)~ THEN BEGIN 25 // from:
  ...
  IF ~  Dead("NWSAMUE2")OR(2)CheckStatLT(LastTalkedToBy(Myself),14,CHR)Alignment(Player1,MASK_EVIL)~ THEN REPLY @52 GOTO 28
  IF ~  Dead("NWSAMUE2")CheckStatGT(LastTalkedToBy(Myself),13,CHR)!Alignment(Player1,MASK_EVIL)~ THEN REPLY @53 GOTO 27
  IF ~  Dead("NWSAMUE2")OR(2)CheckStatLT(LastTalkedToBy(Myself),14,CHR)Alignment(Player1,MASK_EVIL)~ THEN REPLY @53 GOTO 28
...
IF ~~ THEN BEGIN 27 // from: 25.2 25.4
  SAY @56
  IF ~  !Dead("NWMELDAN")~ THEN GOTO 30
  IF ~  Dead("NWMELDAN")~ THEN GOTO 31
END

IF ~~ THEN BEGIN 28 // from: 25.3 25.5
  SAY @57
  IF ~  !Dead("NWMELDAN")~ THEN GOTO 30
  IF ~  Dead("NWMELDAN")~ THEN GOTO 32
END
